    ...
    if request.method == 'GET':
        ...
        for i in range(len(Dates)):
            for a in range(len(Received_Dates)):
                if (str(Dates[i]) == str(Received_Dates[a]) and str(Formatted_Times[i]) == str(Received_Times[a]) and str(Total_Available[i]) == str(Received_Total_Available[a]) and str(Currently_Available[i]) == str(Received_Currently_Available[a])):
                    if(str(Received_Add_or_Drop[a]) == "Add"):
                        print("Events_In_Dict: " + str(Events_In_Dict[i]))
                        Index = list(Events_In_Dict[i].keys()).index("Currently Available:")
                        ...
                        if(val_student_names and val_student_emails):
                            ...
                            if(str(Received_student_name).lower() in str(Student_Name).lower()):
                                ...
                                sheet_instance.update_cell(i+2, Index+3, (int(Events_In_Dict[i]["Currently Available:"])-1))
                        ...
                        print("All_Events_In_Dict: " + str(All_Events_In_Dict[i]))
